...
var string = JSON.stringify(jsonMy);
var arr = JSON.parse(string);

function arrMy(mas, typeS, neme) {
	var skills = _.map(mas, typeS);
	if (neme !== undefined ) {
		skills = _.flatten(mas, neme);
		skills = _.map(skills, neme);
	}else{
	...
	return skills.sort(sortArr);
}

console.log('skills:', arrMy(arr,'skills'));
console.log('friends:', arrMy(arr, 'friends', 'name'));

var names = _.sortBy(arr, function(o) {
	return o.friends.length;
});
names = _.map(names, "name");
names = _.reverse(names);

console.log('Names by number of friends:', names);
